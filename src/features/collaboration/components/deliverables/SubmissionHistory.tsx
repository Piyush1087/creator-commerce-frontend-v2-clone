...
export function SubmissionHistory({ versions }: { versions: CollaborationSubmissionVersion[] }) {
  ...
  return <details className="collab-submission-history"><summary>Version history <span>{versions.length} {versions.length === 1 ? "submission" : "submissions"}</span></summary><ol>{[...versions].reverse().map((version, index) => <li key={version.submissionVersionId} className="collab-submission-version"><header><h6>Version {version.versionNumber}</h6>{index === 0 ? <span>Current</span> : null}</header><dl className="collab-facts"><div><dt>Review status</dt><dd>{reviewLabel(version.reviewState)}</dd></div><div><dt>Submitted</dt><dd>{new Date(version.submittedAt).toLocaleString()}</dd></div><div><dt>Asset reference</dt><dd className="collab-evidence-ref">{version.assetRef}</dd></div>{version.creatorNote ? <div><dt>Creator note</dt><dd>{version.creatorNote}</dd></div> : null}{version.brandFeedback ? <div><dt>Brand feedback</dt><dd>{version.brandFeedback}</dd></div> : null}{version.reviewedAt ? <div><dt>Reviewed</dt><dd>{new Date(version.reviewedAt).toLocaleString()}</dd></div> : null}{version.autoApprovedAt ? <div><dt>Auto-approved</dt><dd>{new Date(version.autoApprovedAt).toLocaleString()}</dd></div> : null}</dl></li>)}</ol></details>;
}
