...
export function FeedbackPanel({ detail, busy, actionError, onSubmit }: Props) {
  ...
  return <section className="collab-exec-card collab-feedback" aria-labelledby="collab-feedback-title"><header className="collab-feedback__header"><div><p className="collab-deliverable__eyebrow">Post-completion</p><h5 id="collab-feedback-title">Feedback</h5></div><span className="collab-status-pill">Not an execution stage</span></header><p>Feedback is post-completion activity and does not affect the completed collaboration.</p>
    {feedback.viewerSubmission ? <><Alert tone="success" title="Feedback submitted">Your feedback will remain private until both sides submit or the feedback window closes.</Alert><FeedbackRecord title="Your feedback" submission={feedback.viewerSubmission} /></> : capabilities.has("submit-feedback") ? <form className="collab-command-form collab-feedback__form" onSubmit={submit} aria-busy={busy}><h6>Leave feedback</h6><SelectField label="Rating" value={rating} options={[{ value: "", label: "Choose a rating" }, ...ratingOptions]} onChange={(event) => { setRating(event.target.value); setError(undefined); }} helperText={error} aria-invalid={Boolean(error)} disabled={busy} /><TextField label="Review (optional)" multiline value={reviewText} onChange={(event) => setReviewText(event.target.value)} disabled={busy} />{actionError ? <p className="collab-form-error" role="alert">{actionError}</p> : null}<Button type="submit" disabled={busy} fullWidthOnMobile>{busy ? "Submitting…" : "Submit feedback"}</Button></form> : <p>The feedback submission window is closed.</p>}
