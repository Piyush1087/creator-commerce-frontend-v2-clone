...
export function NegotiationPanel({ detail, busy, onAction }: { detail: CollaborationDetailResponse; busy: boolean; onAction: (action: "accept-proposal" | "accept-counter" | "decline-negotiation") => void }) {
  ...
  return <section className="collab-exec-card"><h4>Negotiation</h4>
    <p>{commercial?.currency ?? ""} {commercial?.brandCounterFee ?? commercial?.applicationProposedFee ?? "—"}</p>
    <p>The Brand may accept the proposal or make one counter-offer.</p>
    ...
    {!capabilities.has("accept-proposal") && !capabilities.has("accept-counter") ? <Alert tone="info" title="Negotiation">{detail.workflow.actionRequiredBy === "BRAND" ? "Waiting for the Brand." : "Waiting for the Creator."}</Alert> : null}
  </section>;
}
