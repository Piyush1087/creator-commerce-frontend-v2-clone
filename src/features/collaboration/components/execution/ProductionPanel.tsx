...
export function ProductionPanel({
  ...
  const resolved = detail.deliverables.filter(
    (item) => item.state === "APPROVED" || item.state === "AUTO_APPROVED",
  ).length;
  return (
    <section aria-label="Production deliverables" className="collab-production">
      <header>
        <h4>Production</h4>
        <p>
          {resolved} of {detail.deliverables.length} deliverables approved
        </p>
      </header>
