...
type Props = {
  ...
  onAuthorize: (deliverableId: string) => void;
  onDecline: (deliverableId: string) => void;
  onSubmitEvidence: (
...
export function PublishingSettlementPanel(props: Props) {
  const complete = props.detail.deliverables.filter(
    (item) =>
      !item.publishingRequired ||
      item.publishing?.state === "PUBLISHING_NOT_REQUIRED" ||
      item.publishing?.state === "COMPLIANCE_VERIFIED",
  ).length;
  return (
    <section
      className="collab-publishing"
      aria-label="Publishing and settlement"
    >
      <header>
        <h4>Publishing &amp; Settlement</h4>
        <p>
          {complete} of {props.detail.deliverables.length} publishing
          requirements complete
        </p>
      </header>
      {props.detail.deliverables.map((item) => (
        <PublishingDeliverableCard
          key={item.deliverableExecutionId}
          detail={props.detail}
          deliverable={item}
          role={props.role}
