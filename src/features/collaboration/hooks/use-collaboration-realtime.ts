...
export function useCollaborationRealtime({ enabled, selectedCollaborationId, onThreadEvent, onInboxEvent, onReconnect }: Options): CollaborationRealtimeStatus {
  ...
  useEffect(() => {
    ...
    const connect = () => { const reconnecting = connectedOnce.current; connectedOnce.current = true; setStatus("connected"); syncRoom(socket, joined, selected.current); if (reconnecting) void handlers.current.onReconnect?.(); };
