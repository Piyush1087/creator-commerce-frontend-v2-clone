...
export const collaborationStageLabel = (stage: CollaborationStage): string =>
  STAGE_LABELS[stage];
export const collaborationStageChip = (stage: CollaborationStage): string =>
  STAGE_LABELS[stage];
export const collaborationLifecycleLabel = (
  lifecycle: CollaborationLifecycle,
): string => LIFECYCLE_LABELS[lifecycle];
export const collaborationPrimaryStatus = (
  lifecycle: CollaborationLifecycle,
...
export const actionRequiredLabel = (actor: CollaborationActor): string =>
  actor === "NONE"
    ...
    : actor === "SYSTEM"
      ...
      : actor === "ADMIN"
        ? "Under review"
        : `Waiting for ${actor === "BRAND" ? "Brand" : "Creator"}`;
