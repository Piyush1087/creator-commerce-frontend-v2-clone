...
describe("mobile bottom navigation active-state accessibility", () => {
  it("uses an approved AA contrast token while retaining the active indicator", () => {
    ...
    expect(
      contrast(token("--color-secondary"), "#ffffff"),
    ...
    expect(html).toContain(
      '<nav class="aurora-bottom-nav" aria-label="Primary">',
    );
  ...
  it.each([390, 767])(
    "keeps the corrected navigation visible at %ipx",
    (width) => {
  ...
  it.each([768, 1440])(
    "does not expose the mobile navigation at %ipx",
    (width) => {
      expect(width).toBeGreaterThan(767);
      expect(shellCss).toMatch(
        /\.aurora-bottom-nav\s*\{[^}]*display:\s*none;/u,
      );
    },
  );
});
